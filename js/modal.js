...
// Cargar datos de comentarios desde localStorage al iniciar la página
  if (localStorage.getItem('kdramas')) {
      kdramas = JSON.parse(localStorage.getItem('kdramas'));
  } else {
      // Si no hay comentarios guardados, inicializar con los dramas predeterminados
      kdramas = localStorage.getItem('preferredLanguage') === 'es' ? kdramasEs : kdramasEn;
  }

  // Actualizar eventos de clic a cada elemento de drama
  dramas.forEach((drama, index) => {
      drama.setAttribute('data-index', index);
      drama.addEventListener('click', () => {
          const selectedDrama = kdramas[index];
          openModal(selectedDrama);
      });
  });

  // Ejecutar al cargar para mostrar la primera letra del username si está disponible
  const usernameInitial = getUsernameInitial();
  if (usernameInitial) {
      const usernameInitialElement = document.querySelector('.username-initial p');
      if (usernameInitialElement) {
          usernameInitialElement.textContent = usernameInitial;
      }
  }
